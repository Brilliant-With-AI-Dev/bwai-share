...
const CHAPTERS=[
 {id:'back',name:'Looking back',turns:[
  ...
  {q:"Before we move on, what else from the quarter should your planning team understand?",a:'Don’t turn this into “some opportunities for better alignment.” I’m excited and frustrated. Both are true.',closing:true,fields:['reason']}
 ]},
 {id:'Executive',name:'Executive',turns:[
  ...
  {q:"Before we move on, what else should the Executive team understand?",a:'The growth and efficiency points belong together. We need a “how can we” mindset.',closing:true,fields:['Executive-avoided']}
 ]},
 {id:'GTM',name:'GTM',turns:[
  ...
  {q:"Before we move on, what else should the GTM team understand?",a:'I’m not saying the data is useless or that I’m unwilling to move. Keep the uncertainty without making it sound like hesitation.',closing:true,fields:['GTM-example']}
 ]},
 {id:'forward',name:'Looking forward',turns:[
  ...
  {q:"Before we finish, what else would you like your planning team to understand?",a:'Nothing else. Let me see the answers together now.',closing:true,fields:[]}
 ]}
];
...
const REVIEW_NOTES={
 reason:{title:"Review your rating",body:"You selected 6, and your explanation says six and a half. Would you like to clarify the difference?",suggestion:RATING_REVISION,explanation:'Keeps both the whole-number rating and your more nuanced assessment.'},
 'GTM-avoided':{title:"Review this wording",body:"“Kill politicking” makes your frustration clear. Would you like to spell out what you want the team to do? You can keep your original wording.",suggestion:POLITICS_REVISION,explanation:'Keeps the concern about internal politics and the urgency. Makes the requested change explicit.'}
};
...
const Model=(()=>{
 ...
 function reduce(s,action){
  ...
  if(action.type==='edit'||action.type==='discuss'){
   ...
   a.approval=null;s.package=null;s.editing={id:a.id,mode:action.type};s.notice="You’re editing this answer. You’ll need to approve again afterward.";
  }
  if(action.type==='save'){
   const a=s.answers.find(a=>a.id===action.id);if(!a||s.editing?.id!==a.id)return s;
   const candidate={...a,text:action.text};if(!valid(candidate)){s.notice="Enter an answer. Ratings must be whole numbers from 1 to 10.";return s;}
   a.text=action.text;a.version++;a.approval=null;a.flag='';a.choice='Edited';a.review='Participant-edited; no automated review';s.editing=null;s.package=null;s.notice="Changes saved. Approve your full submission when you’re ready.";
  }
  if(action.type==='cancel'){s.editing=null;s.notice="No wording changed. Final approval is still needed.";}
  if(action.type==='keep'){
   const a=s.answers.find(a=>a.id===action.id);if(!a||s.editing)return s;
   a.flag='';a.choice='Kept as written';s.notice="Your wording is unchanged. Review and approve the full submission at the end.";
  }
  ...
  if(action.type==='approveAll'&&s.finished&&!s.editing&&s.answers.every(valid)){
   s.answers.forEach(a=>a.approval={version:a.version,text:a.text});s.notice="All answers approved. Demo only: nothing sent.";
  }
